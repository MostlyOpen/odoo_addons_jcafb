...
class DocumentCreateXlsTCP17Wizard(models.TransientModel):
    _name = 'myo.document.create_xls_tcp17.wizard'
    ...
    reference_file_path = fields.Char(
        ...
        default='/opt/openerp/mostlyopen_clvhealth_jcafb/data/TCP17/survey_jcafb_TCP17.xls'
    )

    @api.multi
    def do_document_create_xls_tcp17(self):
        self.ensure_one()
        ...
        for document_reg in self.document_ids:
            ...
            if document_reg.survey_id.title == '[QDH17]' and document_reg.state == 'done':
                ...
                for document_person in document_reg.person_ids:
                    ...
                    for document_person_2 in person_search.document_person_ids:
                        ...
                        if document_person_2.document_id.survey_id.title == '[TCP17]' and \
                           document_person_2.document_id.state == 'draft':

                            filepath = self.dir_path + '/TCP17_' + document_person_2.document_id.code + '.xls'

                            # survey_user_input_line_search = survey_user_input_line_model.search([
                            #     ('user_input_id', '=', document_reg.survey_user_input_id.id),
                            #     ('question_id', '=', survey_question_search.id),
                            # ])

                            print '>>>>>', filepath
                            # print '>>>>>>>>>>', survey_user_input_line_search.value_text
                            ...
                            sheet = _get_sheet(book, 'TCP17')

                            previousCell = _getOutCell(sheet, 10, 5)
                            sheet.write(10, 5, document_person_2.document_id.code)
                            newCell = _getOutCell(sheet, 10, 5)
                            newCell.xf_idx = previousCell.xf_idx

                            QDH17_01_02 = self.get_value('QDH17_01_02', document_reg.survey_user_input_id)
                            previousCell = _getOutCell(sheet, 15, 5)
                            # sheet.write(15, 5, survey_user_input_line_search.value_text)
                            sheet.write(15, 5, str(QDH17_01_02))
                            ...
                            QDH17_02_06 = self.get_value('QDH17_02_06', document_reg.survey_user_input_id)
                            if QDH17_02_06 == 'b) Paciente':
                                row = 53
                            else:
                                row = 54
                            ...
                            previousCell = _getOutCell(sheet, 62, 5)
                            sheet.write(62, 5, 'X')
                            newCell = _getOutCell(sheet, 62, 5)
                            newCell.xf_idx = previousCell.xf_idx
