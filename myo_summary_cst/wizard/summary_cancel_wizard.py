...
class SummaryCancelyWizard(models.TransientModel):
    _name = 'myo.summary.cancel.wizard'

    summary_ids = fields.Many2many('myo.summary', string='Summaries')

    @api.multi
    def do_summary_cancel(self):
        self.ensure_one()
        ...
        for summary_reg in self.summary_ids:
            ...
            if summary_reg.is_address_summary:
                ...
                for address_person in summary_reg.address_id.person_address_ids:
                    ...
                    if address_person.person_id.state == 'selected':
                        address_person.person_id.state = 'revised'

                    summary_search = summary_model.search([
                        ('person_id', '=', address_person.person_id.id),
                        ('is_person_summary', '=', True),
                    ])
                    for sumary_reg_2 in summary_search:
                        sumary_reg_2.state = 'canceled'
                ...
                for address_person in summary_reg.address_id.person_address_ids:
                    ...
                    for lab_test_request in address_person.person_id.lab_test_request_ids:
                        ...
                        lab_test_request.state = 'canceled'
            ...
            if summary_reg.is_person_summary:
                ...
                summary_reg.person_id.state = 'revised'
                summary_reg.address_id.state = 'revised'

                summary_search = summary_model.search([
                    ('address_id', '=', summary_reg.address_id.id),
                    ('is_address_summary', '=', True),
                ])
                summary_search.state = 'revised'
                ...
                for document_person in summary_reg.person_id.document_person_ids:
                    ...
                    if document_person.document_id.name != '[QSF17]':
                        document_person.document_id.state = 'canceled'
