...
class LabTestResultSwabRefreshWizard(models.TransientModel):
    _name = 'myo.lab_test.result.swab.refresh.wizard'
    ...
    lab_test_type = fields.Char(
        'Lab Test Type',
        required=True,
        default=u'JCAFB 2017 - Laboratório - Pesquisa de Enterobius vermicularis'
    )

    @api.multi
    def do_lab_test_result_swab_refresh(self):
        self.ensure_one()

        lab_test_result_swab_model = self.env['myo.lab_test.result.swab']
        lab_test_result_model = self.env['myo.lab_test.result']

        lab_test_result_swab_search = lab_test_result_swab_model.search([])
        lab_test_result_swab_search.unlink()
        ...
        for lab_test_result_reg in lab_test_result_search:
            ...
            if lab_test_result_reg.lab_test_type_id.name == self.lab_test_type:
                ...
                EEV_01_03 = self.get_value('EEV-01-03', lab_test_result_reg)
                ...
                lab_test_result_swab_model.create(values)
